...
public class CalculatorRequestProcessor implements RequestProcessor {
    private static final Logger calcLogger = LogManager.getLogger(CalculatorRequestProcessor.class.getName());
    ...
    public void execute(HttpRequest httpRequest, OutputStream output) throws IOException {
        ...
        int result = a + b;
        calcLogger.trace("От клиента получены значения a = {}, b = {}", a, b);
        String outMessage = a + " + " + b + " = " + result;
        String response = "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\n\r\n<html><body><h1>" + outMessage + "<h1><body><html>";
        output.write(response.getBytes(StandardCharsets.UTF_8));
        calcLogger.trace("Клиенту отправлен результат вычисления {}", result);
    }
}
